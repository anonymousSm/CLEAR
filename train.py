...
def train_fun(args, train_loader, unlabel_train_loader, current_task, fisher={}, prototype={}):
    ...
    if args.method == 'Independent' or current_task == 0:
        ...
        if args.net == 'resnet32':
            ...
            print('Oh! That was no valid models. ')
    ...
    self_criterion = NT_Net(temperature=0.5).cuda()
    ...
    for epoch in range(args.start, args.epochs):
        ...
        dist_an = 0
        # scheduler.step()
        all_train_loader = zip(train_loader, unlabel_train_loader)
        for i, (label_data, unalbel_data) in enumerate(all_train_loader, 0):
            # labeled data
            inputs, labels = label_data
            loss = 0.0
            # wrap them in Variable
            inputs = Variable(inputs.cuda())
            labels = Variable(labels).cuda()
            ...
            # unlabeled data
            inputs_u, labels_u = unalbel_data
            inputs_u = torch.cat([inputs_u[0], inputs_u[1]], dim=0)
            if torch.cuda.is_available():
                inputs_u = inputs_u.cuda(non_blocking=True)
                labels_u = labels_u.cuda(non_blocking=True)
            bsz = labels_u.shape[0]
            optimizer.zero_grad()

            # embedding for output
            _, embed_feat = model(inputs)
            _, embed_u = model(inputs_u)
            f1, f2 = torch.split(embed_u, [bsz, bsz], dim=0)
            features_u = torch.cat([f1.unsqueeze(1), f2.unsqueeze(1)], dim=1)
            loss_u = self_criterion(features_u)
            ...
            if args.loss == 'MSLoss':
                loss += criterion(embed_feat, labels)
                loss += self_criterion(embed_feat, labels)

            else:
                loss_ML, inter_, dist_ap, dist_an = criterion(embed_feat, labels)
                loss += loss_ML
...
if __name__ == '__main__':
    ...
    parser.add_argument('-num_instances', default=10, type=int, metavar='n',
    ...
    parser.add_argument('-net', default='resnet32')
    ...
    for i in range(num_task):
        if i == 0:
            ...
            # print("num_class_per_task: ", num_class_per_task)
            num_class_per_task = int(num_class_per_task)
            class_index = random_perm[args.base +
                                      (i-1)*num_class_per_task:args.base+i*num_class_per_task]
        if args.data == 'cifar100':
            ori_trainfolder = CIFAR100(
                root=traindir, train=True, download=True, transform=transform_train, index=class_index)
            trainfolder = CIFAR100(
                root=traindir, train=True, download=True, transform=TwoCropTransform(transform_train),
                index=class_index)

        else:
            ori_trainfolder = ImageFolder(
                traindir, transform_train, index=class_index)
            trainfolder = ImageFolder(
                traindir, TwoCropTransform(transform_train), index=class_index)

        label_train_loader = torch.utils.data.DataLoader(
            ori_trainfolder, batch_size=args.BatchSize,
            sampler=RandomIdentitySampler(
                ori_trainfolder, num_instances=args.num_instances),
            drop_last=True, num_workers=args.nThreads)

        unlabel_train_loader = torch.utils.data.DataLoader(
            trainfolder, batch_size=args.BatchSize*2,
            drop_last=True, num_workers=args.nThreads, shuffle=True)
        ...
        if args.method == 'EWC' or args.method == 'MAS':
            fisher = train_fun(args, label_train_loader,
                               unlabel_train_loader, i, fisher=fisher)
        else:
            train_fun(args, label_train_loader, unlabel_train_loader, i)
